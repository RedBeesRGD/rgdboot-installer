...
static s32 Haxx_IOSExploit(u32 entrypoint)
{
    u32* mem1;
    s32 ret;
...
static void Haxx_FlushRange(const void* data, u32 length)
{
...
static bool Haxx_CheckBusAccess(void)
{
    if ((read32(HW_AHBPROT) & 0x80000DFE) != 0x80000DFE)
        return false;

    if ((read32(HW_MEMMIRR) & 0x08) != 0x08)
        return false;
